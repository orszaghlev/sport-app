...
update competitions.match m
set m.homeName = (select fullName
                  from competitions.team t
                  where t.id = m.homeTeam);

update competitions.match m
set m.awayName = (select fullName
                  from competitions.team t
                  where t.id = m.awayTeam);

update competitions.event e
set e.athleteName = (select a.name
                     from competitions.athlete a
                     where a.id = e.athleteId);

update competitions.event e
set e.teamShortName = (select shortName
                     from competitions.team t
                     where t.id = e.teamId);
